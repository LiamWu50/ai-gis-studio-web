...
- 当前支持文件: `.geojson`、`.json`
- 当前不支持: Shapefile zip、在线 URL、PostGIS 图层、WMS/WFS/WMTS/XYZ 服务注册
...
2. 上传成功后保存返回的 `InputDataSummary` 到数据上下文面板。
3. 地图预览使用 `GET /api/datasets/{datasetId}/preview`，不要用上传响应里的 `dataRef` 直接取文件。
4. 如果 `warnings` 非空，在数据详情里展示给用户。
5. 后续 Agent 请求只传 `datasetId`，不要传完整 GeoJSON。
